...
/// Attributes for the `#[parser_methods]` macro.
pub(crate) struct ParserMethodsAttrs {
    pub(crate) rule_enum: Path,
}

impl Parse for ParserMethodsAttrs {
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        Ok(ParserMethodsAttrs { rule_enum })
    }
}
...
pub(crate) struct AliasSrc {
    pub(crate) ident: Ident,      // Identifier
    pub(crate) is_shortcut: bool, // Whether it's a shortcut
}

/// Parsed function metadata including function body, name, input argument, and aliases.
pub(crate) struct ParsedFn<'a> {
...
///
/// When `allow_aliasing` is false, any `#[alias(...)]` attribute is rejected: a caller that
/// only sees one of several impl blocks for a type (as `#[parser_methods]` does) can't build a
/// matching `AliasedRule` enum for it, since that requires seeing every rule method up front.
pub(crate) fn collect_aliases(
        ...
        if let Some(attr) = alias_attrs.next() {
            if !allow_aliasing {
                return Err(Error::new(
                    attr.span(),
                    "`#[alias(...)]` is not supported inside a `#[parser_methods]` impl block, since \
                     `declare_parser!` cannot see every rule method to build a matching `AliasedRule` \
                     enum. Use a single `#[parser]` impl block instead if you need rule aliasing.",
                ));
            }
    ...
    debug_assert!(
        !alias_map.is_empty(),
        "Alias map should not be empty after collection"
    );
    Ok(alias_map)
}
...
/// Rewrites every rule method in `imp` to add shortcut/alias dispatch, consuming matched
/// entries out of `alias_map`, then appends a synthesized dispatcher function for any
/// remaining alias target that isn't itself a real method.
///
/// Shared by the legacy `#[parser]` macro (single impl block, emits its own `AliasedRule`
/// enum) and `#[parser_methods]` (one of several impl blocks for a type declared once via
/// `declare_parser!`).
    ...
    let fn_map: BTreeMap<Ident, ParsedFn> = imp
        ...
        .map(|method| {
            *method = parse_quote!(
            ...
            let mut f = parse_fn(method, alias_map)?;
...
/// Main function for generating a `#[parser_methods]` impl block: rewrites its rule methods
/// for shortcut/alias dispatch, without emitting an `AliasedRule` enum or `Parser` impl. Pair
/// with a single `declare_parser!` invocation for the type.
pub fn make_parser_methods(
    attrs: proc_macro::TokenStream,
    input: proc_macro::TokenStream,
) -> Result<proc_macro2::TokenStream> {
    let attrs: ParserMethodsAttrs = syn::parse(attrs)?;
    let rule_enum = &attrs.rule_enum;
    let mut imp: ItemImpl = syn::parse(input)?;

    let mut alias_map = collect_aliases(&mut imp, false)?;
    process_methods(&mut imp, rule_enum, &mut alias_map)?;
